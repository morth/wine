...
static void update_visible_region( struct dce *dce )
{
    ...
    HRGN vis_rgn = 0;
    HWND top = 0;
    struct quartzdrv_escape_set_drawable escape;
    struct quartzdrv_win_data *windata;
    ...
    do
    {
        ...
        SERVER_START_REQ( get_visible_region )
        {
            ...
            if (!(status = wine_server_call( req )))
            {
                ...
                top = reply->top_win;
                escape.org.x = reply->win_rect.left - reply->top_rect.left;
                escape.org.y = reply->win_rect.top - reply->top_rect.top;
                escape.drawable_org.x = reply->top_rect.left;
                escape.drawable_org.y = reply->top_rect.top;
            }
    ...
    if (dce->clip_rgn) CombineRgn( vis_rgn, vis_rgn, dce->clip_rgn,
     ...
     if (top == dce->hwnd && ((windata = get_win_data( dce->hwnd )) != NULL) &&
         IsIconic( dce->hwnd ) && windata->icon_window)
    {
        escape.drawable = windata->icon_window;
     
     }
     else
     {
        escape.drawable = QDRV_get_whole_window( top );
    }
    
    escape.code = QDRV_SET_DRAWABLE;
    
    //escape.mode = IncludeInferiors;
    
    ExtEscape( dce->hdc, QDRV_ESCAPE, sizeof(escape), (LPSTR)&escape, 0, NULL );
...
{
    struct quartzdrv_escape_set_dce escape;
    struct dce *dce;
    ...
    escape.code = QDRV_SET_DCE;
    escape.dce  = dce;
    ExtEscape( dce->hdc, QDRV_ESCAPE, sizeof(escape), (LPCSTR)&escape, 0, NULL );
...
*/
void alloc_window_dce( struct quartzdrv_win_data *data )
{
    struct quartzdrv_escape_set_dce escape;
    struct dce *dce;
    void *class_ptr = NULL;
...
*/
void free_window_dce( struct quartzdrv_win_data *data )
{
...
*/
HDC QDRV_GetDCEx( HWND hwnd, HRGN hrgnClip, DWORD flags )
{
    ...
    TRACE("hwnd %p, hrgnClip %p, flags %08lx data %p\n", hwnd, hrgnClip, flags, data);
...
/***********************************************************************
*		QDRV_ReleaseDC  (X11DRV.@)
*/
INT QDRV_ReleaseDC( HWND hwnd, HDC hdc, BOOL end_paint )
{
    enum quartzdrv_escape_codes escape = QDRV_GET_DCE;
    struct dce *dce;
    BOOL ret = FALSE;
    ...
    EnterCriticalSection( &dce_section );
    if (!ExtEscape( hdc, QDRV_ESCAPE, sizeof(escape), (LPCSTR)&escape,
                    sizeof(dce), (LPSTR)&dce )) dce = NULL;
    if (dce && dce->count)
    {
        if (end_paint || (dce->flags & DCX_CACHE))
        {
#ifdef WINE_COCOA
            struct quartzdrv_win_data *data;
            if ( (data = get_win_data(hwnd)) ) 
                Cocoa_NeedDisplay(data->whole_window);
#endif
            delete_clip_rgn( dce );
        }
...
*		WindowFromDC   (X11DRV.@)
*/
HWND QDRV_WindowFromDC( HDC hdc )
{
    enum quartzdrv_escape_codes escape = QDRV_GET_DCE;
    struct dce *dce;
    HWND hwnd = 0;
