...
WINE_DEFAULT_DEBUG_CHANNEL(quartzsettings);
...
static DWORD PASCAL X11DRV_Settings_SetMode(LPDDHAL_SETMODEDATA data)
{
    ...
    QDRV_DDHAL_SwitchMode(data->dwModeIndex, NULL, NULL);
    data->ddRVal = DD_OK;
...
int X11DRV_Settings_CreateDriver(LPDDHALINFO info)
{
    ...
    QDRV_DDHAL_SwitchMode(pGetCurrentMode(), NULL, NULL);
...
/* 
    FIXME
    import from x11ddraw.c only here to build settings.c
*/
void QDRV_DDHAL_SwitchMode(DWORD dwModeIndex, LPVOID fb_addr, LPVIDMEM fb_mem)
{
    FIXME("\n");
/*
